...
const ChallengeFeed: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Restore card index if returning from a feed
    const savedIndex = localStorage.getItem('community_current_index');
    if (savedIndex) {
      const index = parseInt(savedIndex);
      if (!isNaN(index)) {
        setCurrentIndex(index);
        localStorage.removeItem('community_current_index'); // Clear after use
      }
    }
  }, []);
  ...
  const fetchInteractions = async () => {
    try {
      ...
      for (const challenge of challengesData) {
        ...
        // Priority 2: Recent validated post (completion)
        const latestValidatedPost = posts.find(post => post.verified === true);
        if (latestValidatedPost) {
          const profile = (latestValidatedPost.profiles as any) || profilesMap[latestValidatedPost.user_id];
          interactions.push({
            id: latestValidatedPost.id,
            challenge_id: challenge.id,
            challenge_title: challenge.title,
            challenge_image: challenge.image_url,
            category_name: challenge.challenge_categories?.name || 'Challenge',
            ...
            comments_count: totalComments,
            completion: {
              content: latestValidatedPost.content,
              user_name: profile?.username || 'Unknown',
              user_avatar: profile?.avatar_url || null,
              created_at: latestValidatedPost.created_at,
              verified: latestValidatedPost.verified,
            },
          });
          continue;
        }
  ...
  const handleViewInteraction = (interaction: ChallengeInteraction) => {
    // Save current card index to localStorage so we can return to it
    localStorage.setItem('community_current_index', currentIndex.toString());
    // Navigate to the new feed page for this challenge with the interaction ID to highlight
    navigate(`/feed/${interaction.challenge_id}?highlight=${interaction.id}`);
  };
